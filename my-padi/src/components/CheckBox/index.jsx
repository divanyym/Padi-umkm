...
const Button = ({
    ...
    return (
        <button
            className={`${className} flex flex-row items-center justify-center text-center cursor-pointer whitespace-nowrap ${shape ? shapes[shape] : ''} ${size ? sizes[size] : ''} ${variant && color ? variants[variant][color] : ''}`}
            {...restProps}
        >
            {!!leftIcon && leftIcon}
            {children}
